...
data_folder = os.path.join(root_dir, "data", "chexpert")
os.makedirs(data_folder, exist_ok=True)

# Load data
print("Loading data...")
data_df = pd.read_csv(os.path.join(data_folder, 'metadata.csv'))

with open(os.path.join(data_folder, 'embed.pkl'), "rb") as f:
    emb_df = pickle.load(f)
data_df = pd.merge(data_df, emb_df, on='Path', how='inner')
...
print(data_df["age_decile"])

# data_df=data_df.reset_index()
# metadata to analyze
metadata = ["grouped_race", "gender", "age_decile", "No Finding"]
metadata = ["age_decile", "No Finding"]

wo = False
alpha = 0.05
len_emb = 1376
results_folder = "chexpert/cxr-foundation"
