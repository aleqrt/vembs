...
embedding_folder = os.path.join(chexpert_data_folder, "cxr-foundation")
extracted_embedding_folder = os.path.join(chexpert_data_folder, "cxr-foundation")
...
embed_pkl = os.path.join(embedding_folder, "embed.pkl")
...
print("Loading CheXpert embeddings...")
...
# ----------------------------------------------------------------
# LOAD MEDCLIP PICKLE TO GET THE PATHS
# ----------------------------------------------------------------
medclip_folder = os.path.join(chexpert_data_folder, "medclip")
medclip_pickle_file = os.path.join(medclip_folder, "embedding_from_medclip.pkl")
print("Loading MedCLIP embeddings from:", medclip_pickle_file)
with open(medclip_pickle_file, 'rb') as f:
    medclip_embed_tuple = pickle.load(f)

# The medclip pickle tuple structure:
#   element 3: list of paths
medclip_paths_list = medclip_embed_tuple[3]


def extract_path(p):
    """Recursively extract the innermost element from a nested list and return it as a string,
    removing the '-small' substring."""
...
# Process MedCLIP paths
medclip_paths = [extract_path(p) for p in medclip_paths_list]
print(f"Loaded {len(medclip_paths)} MedCLIP paths.")

# ----------------------------------------------------------------
# FILTER THE CXR-FOUNDATION DATASET USING MEDCLIP PATHS
# ----------------------------------------------------------------
print("Filtering cxr-foundation data based on MedCLIP paths...")
# Here we retain only the instances whose 'path' is found in the MedCLIP paths list.
df_filtered = df_merged[df_merged['path'].isin(medclip_paths)]
print("Filtered cxr-foundation data shape:", df_filtered.shape)
...
for meta in metas:
    ...
    models_folder = os.path.join(root_dir, "models", "chexpert", "cxr-foundation", meta)
    figures_folder = os.path.join(root_dir, "fig", "chexpert", "cxr-foundation", f"predict-{meta}")
