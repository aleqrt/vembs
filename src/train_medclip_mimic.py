...
from utils import plot_loss_acc_cv, remove_correlated_features, \
    get_embedding_from_medclip_and_metadata, plot_roc_curves, plot_boxplot
...
medclip_embedding_folder = os.path.join(root_dir, "data", "mimic", "medclip-embedding")
medclip_embedding_path = os.path.join(medclip_embedding_folder, "embedding_from_medclip.pkl")
train_test_idx_path_medclip = os.path.join(medclip_embedding_folder, "train_test_idx.pkl")
...
for metadata in metas:
    ...
    # Path for saving train/test split
    ...
    if os.path.exists(train_dataset_path):
        ...
        if os.path.exists(medclip_embedding_path):
            ...
            print("Load MedCLIP...")
            # Replace MedCLIPProcessor with CLIPImageProcessor
            ...
            # Manually load the state_dict with strict=False to ignore unexpected keys
            model_path = os.path.join(root_dir, 'pretrained', 'medclip-vit', 'pytorch_model.bin')
            ...
            X, y, valid_id, ids = get_embedding_from_medclip_and_metadata(data_df, root_dir, model,
                                                                          preprocess, device)

            print('Saving embedding and target variables in raw format...')
            with open(medclip_embedding_path, 'wb') as f:
        ...
        train_all_ids_f = [value for i, value in enumerate(ids) if value in train_ids and i in valid_id]
        test_all_ids_f = [value for i, value in enumerate(ids) if value in test_ids and i in valid_id]
    ...
    for model_name, model in models.items():
        ...
        for i, (train_ids_idx, val_ids_idx) in enumerate(kf.split(train_ids), 1):
            print(f'Fold {i}...')
            train_index = [j for j, value in enumerate(train_all_ids_f) if value in train_ids[train_ids_idx]]
            val_index = [j for j, value in enumerate(train_all_ids_f) if value in train_ids[val_ids_idx]]
            ...
            results[model_name]['f1'].append(float(f1_score(y_val_fold, y_pred, average='weighted', zero_division=0)))
