...
importance_folder = os.path.join(root_dir, "data", "mimic", embedding, "task-analysis-xgb")
...
for metadata in metas:
    ...
    # Load all XGBoost models for the current metadata
    models[metadata] = []
    for fold in range(1, 11):
        model_path = os.path.join(models_folder, f'xgboost_fold_{fold}.pkl')

        model = joblib.load(model_path)
        models[metadata].append(model)
    print(f"{len(models[metadata])} XGBoost models for {metadata} loaded and stored.")
    ...
    if os.path.exists(importance_file):
        ...
        for model in models[metadata]:
            ...
            importances_all_models.append(importances)
...
def retrain_xgb_models(models_list, X_train, y_train, n_splits=10):
    ...
    # Initialize lists to store retrained models and accuracy for each fold
    retrained_models = []
    fold_accuracies = []
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train)):
        ...
        # Update parameters to match the task type (binary or multi-class)
        cloned_model.set_params(objective=objective, eval_metric=eval_metric)
        if n_classes > 2:
            cloned_model.set_params(num_class=n_classes)

        # Retrain model with additional boosting rounds
        xgb_model = XGBClassifier(
            **cloned_model.get_params()
        )
        ...
        retrained_models.append(xgb_model)
...
def evaluate_performance_drop(metadata, k_values):
    ...
    for k in k_values:
        ...
        if k == 0:
            ...
            retrained_models[metadata] = retrain_xgb_models(models[metadata], X_train_modified, y_train)
            retrained_models['disease'] = retrain_xgb_models(models['disease'], X_train_disease_modified,
                                                             y_train_disease)
